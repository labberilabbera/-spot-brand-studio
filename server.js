...
const LOGIN_HTML = [
  ...
  '<title>spot. Brand Studio</title>',
  '<style>*{box-sizing:border-box;margin:0;padding:0}',
  'body{font-family:Segoe UI,sans-serif;background:#0f0f0f;min-height:100vh;display:flex;align-items:center;justify-content:center}',
  '.card{background:#fff;border-radius:20px;padding:40px 36px;width:min(380px,92vw);box-shadow:0 24px 60px rgba(0,0,0,.4)}',
  '.logo{font-size:28px;font-weight:800;color:#b31e59;margin-bottom:4px}',
  '.tag{font-size:13px;color:#9ca3af;margin-bottom:32px}',
  '.field{margin-bottom:16px}',
  'label{display:block;font-size:12px;font-weight:600;color:#374151;margin-bottom:6px}',
  'input{width:100%;padding:11px 14px;border:1.5px solid #e5e7eb;border-radius:10px;font-size:15px;outline:none;font-family:inherit;transition:border .2s}',
  'input:focus{border-color:#b31e59}',
  '.err{color:#b31e59;font-size:13px;margin-top:8px;display:none}.err.show{display:block}',
  'button{width:100%;margin-top:8px;padding:13px;background:#b31e59;color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:600;cursor:pointer;font-family:inherit}',
  '</style></head><body>',
  '<div class="card"><div class="logo">spot.</div><div class="tag">content studio</div>',
  '<form method="POST" action="/login">',
  '<div class="field"><label>Användarnamn</label><input type="text" name="username" placeholder="Ange användarnamn" autofocus autocomplete="username"/></div>',
  '<div class="field"><label>Lösenord</label><input type="password" name="password" placeholder="Ange lösenord"/></div>',
  '<div class="err" id="err">Fel användarnamn eller lösenord.</div>',
